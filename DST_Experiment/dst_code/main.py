...
def main():
    ...
    def compute_metrics(eval_preds):
        def paser(sent):
            belief_state = []
            origin_sent = sent.replace(' ', '')
            domain_slot_values = origin_sent.split(',')
            domain_slot_values = [x.strip() for x in domain_slot_values]
            for item in domain_slot_values:
                try:
                    domain, slot, value = item.split('-', 2)
                except:
                    continue
                if value == '无':
                    continue
                belief_state.append('-'.join([domain, slot, value]).strip())
            return set(belief_state)
        def joint_accuracy(preds, labels):
            ...
            for i in range(num_turns):
                pred = paser(preds[i])
                label = paser(labels[i])
                if pred == label:
                    joint_acc += 1
            joint_acc = joint_acc*1.0 / num_turns
            return joint_acc
        ...
        joint_acc = joint_accuracy(decoded_preds, decoded_labels)
        result = {}
        prediction_lens = [np.count_nonzero(pred != tokenizer.pad_token_id) for pred in preds]
        result["gen_len"] = np.mean(prediction_lens)
        result["joint_acc"] = joint_acc
        return result
